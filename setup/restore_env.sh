...
if [ -d "$UV_BIN_DIR" ]; then
    ...
    echo "✓ Added persistent uv to PATH."
else
    echo "⚠ uv not found. Please run 'setup/setup_uv_env.sh' first."
fi
...
if [ -d "$OLLAMA_BIN_DIR" ]; then
    ...
    echo "✓ Added persistent ollama to PATH."
else
    echo "⚠ ollama not found. Please run 'setup/setup_ollama.sh' first."
fi

# 4. Ollama モデルの場所を設定 (setup_ollama.sh が設定したパス)
export OLLAMA_MODELS="$HOME/work/.ollama-models"
echo "✓ Set OLLAMA_MODELS to $OLLAMA_MODELS"

# 5. MeCab へのパスを通す (setup_mecab.sh が配置したパス)
...
if [ -f "$MECAB_BIN_DIR/mecab" ]; then
    ...
    echo "✓ Added persistent MeCab to PATH."
else
    # MeCabはオプションなので警告のみ
    echo "ⓘ MeCab not found (optional)."
fi
...
if [ -f "$VENV_DIR/bin/activate" ]; then
    ...
    echo "⚠ .venv not found. Please run 'setup/setup_uv_env.sh' first."
fi
...
if curl -s http://localhost:11434/api/tags > /dev/null; then
    ...
    # 8. curlで起動確認
    if curl -s http://localhost:11434/api/tags > /dev/null; then
        echo "✓ Ollama server started successfully and API is accessible."
    else
