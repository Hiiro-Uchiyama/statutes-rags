#!/usr/bin/env python3
"""
Baseline RAG - Hybrid検索 + Reranker

エージェントアーキテクチャなしの単純なRAGベースライン
Hybrid (Vector + BM25 RRF) + Cross-encoder Rerankerを使用
"""
import os
import sys
import json
...
def baseline_hybrid_reranker_answer(llm, retriever, reranker, question: str, choices: List[str]) -> Dict[str, Any]:
    """Hybrid + Reranker RAG"""
...
def main():
    print("=" * 60)
    print("Baseline RAG Evaluation (Hybrid + Reranker)")
    print("=" * 60)
    ...
    results_file = results_dir / "baseline_hybrid_reranker_results.json"
    
    results = {
        "method": "Baseline RAG (Hybrid + Reranker)",
        "config": {
            "index": INDEX_PATH,
            "top_k": TOP_K,
    ...
    for i, q in enumerate(questions[start_idx:], start=start_idx):
        ...
        try:
            result = baseline_hybrid_reranker_answer(llm, retriever, reranker, question, choices)
    ...
    print()
    print("=" * 60)
    print(f"Baseline Hybrid + Reranker Results")
    print("=" * 60)
