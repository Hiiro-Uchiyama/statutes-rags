...
from proposed_workflow_v5 import ProposedWorkflowV5, WorkflowConfig
...
def main():
    print("=" * 60)
    print("Proposed v5: Adaptive Multi-Agent Workflow")
    print("=" * 60)
    print(f"Model: {LLM_MODEL}")
    print(f"Index: faiss_index_xml_v2")
    print(f"Search: Hybrid (Vector + BM25) + Reranker")
    print(f"Features: Complexity-based routing, Numeric comparison, Related search")
    print()
    ...
    # ワークフロー初期化
    print("Initializing v5 workflow...")
    ...
    workflow = ProposedWorkflowV5(retriever=retriever, config=config)
    ...
    # 結果ファイル
    results_file = RESULTS_DIR / "proposed_v5_results.json"
    log_file = RESULTS_DIR / "proposed_v5_run.log"
    ...
    # 戦略別カウント
    strategy_counts = {'simple_rag': 0, 'enhanced_rag': 0, 'full_multi_agent': 0}
    strategy_correct = {'simple_rag': 0, 'enhanced_rag': 0, 'full_multi_agent': 0}
    ...
    with open(log_file, "a") as log:
        ...
        for sample in questions:
            ...
            print(f"{status} ({answer}/{correct_answer}) [{strategy}] {elapsed:.1f}s")
            
            log.write(f"Q{q_id}: {status} ({answer}/{correct_answer}) [{strategy}] {elapsed:.1f}s\n")
            log.flush()
            ...
            if (q_id + 1) % 5 == 0:
                ...
                print(f"  戦略別: simple={strategy_counts['simple_rag']}, enhanced={strategy_counts['enhanced_rag']}, full={strategy_counts['full_multi_agent']}")
                
                with open(results_file, "w") as f:
                    json.dump({
                        "experiment": "proposed_v5",
                        "model": LLM_MODEL,
    ...
    print(f"\n戦略別精度:")
    for s in ['simple_rag', 'enhanced_rag', 'full_multi_agent']:
        if strategy_counts[s] > 0:
            acc = strategy_correct[s] / strategy_counts[s] * 100
            print(f"  {s}: {strategy_correct[s]}/{strategy_counts[s]} ({acc:.1f}%)")
    print(f"\n結果: {results_file}")
