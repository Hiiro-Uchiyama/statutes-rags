#!/usr/bin/env python3
"""
Baseline RAG - 純粋なRAG (Vector検索 + LLM直接回答)
...
def main():
    ...
    results_file = results_dir / "baseline_rag_results.json"
    
    # 既存結果の読み込み（再開用）
    results = {
        "method": "Baseline RAG (Vector + LLM)",
        "config": {
            "index": INDEX_PATH,
            "top_k": TOP_K,
            "model": LLM_MODEL,
