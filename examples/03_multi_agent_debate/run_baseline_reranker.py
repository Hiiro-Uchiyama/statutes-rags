#!/usr/bin/env python3
"""
Baseline RAG - Vector検索 + Reranker

エージェントアーキテクチャなしの単純なRAGベースライン
Cross-encoder Rerankerを使用
"""
import os
import sys
import json
...
# 設定
TOP_K = 30  # 検索候補数
RERANK_TOP_N = 15  # Reranker後の上位件数
TIMEOUT = 180
INDEX_PATH = "data/faiss_index_lawqa/vector"
LLM_MODEL = "qwen3:8b"
NUM_CTX = 30000
...
def baseline_reranker_answer(llm, retriever, reranker, question: str, choices: List[str]) -> Dict[str, Any]:
    """Vector + Reranker RAG"""
    ...
    # Step 2: Reranking
    reranked_docs = reranker.rerank(question, docs, top_n=RERANK_TOP_N)
...
def main():
    print("=" * 60)
    print("Baseline RAG Evaluation (Vector + Reranker)")
    print("=" * 60)
    ...
    # Retriever初期化
    vector_path = str(project_root / INDEX_PATH)
    retriever = VectorRetriever(
        embedding_model="intfloat/multilingual-e5-large",
        index_path=vector_path
    )
    retriever.load_index()
    print(f"Vector index loaded: {retriever.vector_store.index.ntotal} docs")
    ...
    results_file = results_dir / "baseline_reranker_results.json"
    
    results = {
        "method": "Baseline RAG (Vector + Reranker)",
        "config": {
            "index": INDEX_PATH,
            "top_k": TOP_K,
    ...
    for i, q in enumerate(questions[start_idx:], start=start_idx):
        ...
        try:
            result = baseline_reranker_answer(llm, retriever, reranker, question, choices)
    ...
    print()
    print("=" * 60)
    print(f"Baseline Vector + Reranker Results")
    print("=" * 60)
