...
質問が曖昧であっても、提示された選択肢（a/b/c/d）を必ずすべて検討し、法令根拠に基づいて最も妥当な選択肢を特定してください。

【出力形式と制約】
- 無意味な記号・絵文字・外国語文字を一切使用しない。
- 見出し「主張」「推論」「引用条文」をこの順番で必ず含める。
- 推論では各選択肢を必ず評価し、次のテンプレートに厳密に従う：
  ```
  推論：
  - a) 適合/不適合: <理由（法令根拠を簡潔に記載）>
  - b) 適合/不適合: <理由>
  - c) 適合/不適合: <理由>
  - d) 適合/不適合: <理由>
  - 最有力選択肢: <a/b/c/d>
  ```
- 引用条文では根拠とした法令名と条番号を列挙する（該当がない場合は「該当条文なし」と書く）。
- 必ず1つの選択肢だけを最有力として明記する。
