#!/usr/bin/env python3
"""
アブレーション実験: XML v2 + Hybrid + CoTプロンプト + Reranker
...
- run_ablation_xml_cot.py (CoT プロンプトのみ)
"""
import os
import sys
import json
...
def rerank_documents(reranker, query: str, docs, top_n: int = 15):
...
def ablation_answer(llm, retriever, reranker, question: str, choices: List[str]) -> Dict[str, Any]:
    """アブレーション実験用RAG回答（Reranker付き）"""
    ...
    # Step 4: Rerank
    docs = rerank_documents(reranker, question, docs, top_n=RERANK_TOP_N)
...
def main():
    print("=" * 60)
    print("Ablation: XML v2 + Hybrid + CoT Prompt + Reranker")
    print("=" * 60)
    ...
    # 結果ファイル
    results_file = RESULTS_DIR / "ablation_xml_cot_rerank_results.json"
    log_file = RESULTS_DIR / "ablation_xml_cot_rerank_run.log"
    ...
    with open(log_file, "a") as log:
        ...
        for sample in questions:
            ...
            result = ablation_answer(llm, retriever, reranker, question, choices)
            ...
            if (q_id + 1) % 5 == 0:
                ...
                with open(results_file, "w") as f:
                    json.dump({
                        "experiment": "ablation_xml_cot_rerank",
                        "model": LLM_MODEL,
